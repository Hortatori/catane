	...
	public void VoleurArrive(ArrayList<Joueur> joueurs, Joueur leader) {
		...
		for (Joueur joueur : joueurs) {
			if (joueur.getR().total() >= 7) {
				...
				leader.partie.Communicate(
						joueur.getNom() + ", vous �tes trop riche, vous avez " + joueur.getR().total() + " ressources",
						false);
				...
				while (aDefausser > 0) {
					...
					if (!leader.partie.plateau.graphique) {
						...
						VueJoueur.RessourcePanel rp = joueur.vj.new RessourcePanel(
								"Choisissez une ressource � d�fausser", this);
						joueur.vj.add(rp);
						aDefausser--;
				...
				leader.partie
						.Communicate(joueur.getNom() + " vous n'�tes pas assez riche pour que le voleur vous frappe");
			}
		...
		if (leader.isIa()) {
			...
			if (!partie.plateau.graphique) {
				coord = this.partie.ij.getCoord("Joueur " + leader.getNom()
						+ ", vous decidez quelle sera la prochaine place du Voleur! Donnez des coordonn�es (entre 0 et 3)");
				VoleurPart(coord);
			} else {
				VueJoueur.FuiteVoleurPanel cp = leader.vj.new FuiteVoleurPanel("D�placez le voleur o� bon vous semble");
				leader.vj.add(cp);
